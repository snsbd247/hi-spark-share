...
export default function ProfitLoss() {
  ...
  const { data: accounts = [] } = useQuery({
    ...
    queryFn: async () => { const { data } = await apiDb.from("accounts").select("*").in("type", ["income", "expense"]).eq("is_active", true).order("code"); return data || []; },
  });

  const { data: transactions = [] } = useQuery({
    queryKey: ["transactions-pl", dateFrom, dateTo],
    queryFn: async () => {
      let q = apiDb.from("transactions").select("account_id, debit, credit");
