...
export default function TrialBalance() {
  ...
  const { data: transactions = [] } = useQuery({
    ...
    queryFn: async () => {
      let q = ( db as any).from("transactions").select("account_id, debit, credit, date");
  ...
  const { data: openingTxns = [] } = useQuery({
    ...
    queryFn: async () => {
      if (!dateFrom) return [];
      const { data } = await ( db as any).from("transactions").select("account_id, debit, credit").lt("date", dateFrom);
      return data || [];
    },
  });
