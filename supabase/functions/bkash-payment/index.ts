...
const DEFAULT_BASE_URL = "https://tokenized.pay.bka.sh/v1.2.0-beta";
...
async function requireGatewayConfig(): Promise<{ gw: any; error?: Response }> {
  const gw = await getGatewayConfig();
  if (!gw || !gw.app_key || !gw.app_secret || !gw.username || !gw.password) {
    return {
      gw: null,
      error: new Response(
        JSON.stringify({ error: "bKash gateway not configured. Please configure credentials in Settings → bKash API." }),
...
async function getTokenFromGateway(gw: any): Promise<string> {
  const baseUrl = gw.base_url || DEFAULT_BASE_URL;
...
async function handleTestConnection(): Promise<Response> {
  ...
  const { gw, error } = await requireGatewayConfig();
  if (error) { await logRequest("test_connection", "failed", "Gateway not configured"); return error; }

  try {
    await getTokenFromGateway(gw);
    const supabase = getSupabaseAdmin();
...
async function handleRefund(body: any): Promise<Response> {
  ...
  try {
    ...
    const res = await fetch(`${baseUrl}/tokenized/checkout/payment/refund`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: token, "X-APP-Key": gw.app_key },
      body: JSON.stringify({ paymentID, trxID, amount: amount.toString(), reason: reason || "Customer refund", sku: "ISP-REFUND" }),
    });
    ...
    if (data.transactionStatus === "Completed" || data.statusCode === "0000") {
      const supabase = getSupabaseAdmin();
      await supabase.from("payments").update({ status: "refunded" }).eq("bkash_payment_id", paymentID);
    }
...
async function handleCreate(body: any): Promise<Response> {
  ...
  const { gw, error: gwError } = await requireGatewayConfig();
  if (gwError) return gwError;

  const supabase = getSupabaseAdmin();
  const { data: bill, error: billError } = await supabase
    .from("bills").select("amount, customer_id, status").eq("id", bill_id).single();
  ...
  const res = await fetch(`${baseUrl}/tokenized/checkout/create`, {
    ...
    headers: { "Content-Type": "application/json", Authorization: token, "X-APP-Key": gw.app_key },
    body: JSON.stringify({
      mode: "0011", payerReference: customer_id, callbackURL: callback_url,
      amount: amount.toString(), currency: "BDT", intent: "sale", merchantInvoiceNumber: invoiceNumber,
    }),
  });
  ...
  if (data.bkashURL) {
    await supabase.from("payments").insert({
      customer_id, bill_id, amount, payment_method: "bkash", status: "pending",
      bkash_payment_id: data.paymentID, month: null, transaction_id: invoiceNumber,
    });
...
async function handleExecute(body: any): Promise<Response> {
  ...
  if (data.statusCode === "0000" && data.transactionStatus === "Completed") {
    const { data: payment } = await supabase
      .from("payments").select("bill_id, customer_id, amount").eq("bkash_payment_id", paymentID).single();

    const bkashAmount = parseFloat(data.amount);
    if (payment && Math.abs(bkashAmount - Number(payment.amount)) > 0.01) {
      await supabase.from("payments").update({ status: "failed", bkash_trx_id: data.trxID }).eq("bkash_payment_id", paymentID);
    ...
    await supabase.from("payments").update({
      status: "completed", bkash_trx_id: data.trxID, transaction_id: data.trxID, paid_at: new Date().toISOString(),
    }).eq("bkash_payment_id", paymentID);

    if (payment?.bill_id) {
      const { data: bill } = await supabase.from("bills").select("month").eq("id", payment.bill_id).single();
      await supabase.from("bills").update({ status: "paid", paid_date: new Date().toISOString() }).eq("id", payment.bill_id);
      if (bill?.month) {
        await supabase.from("payments").update({ month: bill.month }).eq("bkash_payment_id", paymentID);
      }
    }
  ...
  await supabase.from("payments").update({ status: "failed" }).eq("bkash_payment_id", paymentID);
